        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转到B路由；这里在加入购物车，进行路由跳转之前，发请求把你购买的商品信息给服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      // 发请求，将商品加入到数据库
      /*
      当前这里是派发一个action，向服务器发请求，判断加入购物车成功还是失败，进行相应的操作，
      this.$store.dispatch('addOrUpdateShopCar', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
      上面这段代码，就是调用仓库中的addOrUpdateShopCar函数，返回的是一个Promise，要么成功，要么失败
      */
      try {
        await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
        // 路由跳转并需要将商品的信息带给下一级路由组件
        // 下面这种手段路由跳转以及传递参数可行，一般简单参数通过query传递，复杂参数（skuInfo）通过会话存储到本地
