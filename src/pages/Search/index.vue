    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
          ...
          <!-- 分页器 -->
          <ShopPagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="total" :continues="5" @getPageNo="getPageNo"></ShopPagination>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeMount() {
    ...
    // 简单的写法，ES6新增语法，将其余对象中属性的值赋给目标对象中相同属性名的值，并返回赋值后的目标对象
  ...
  computed: {
    // mapGetters里面的写法：传递的是数组，因为getters是没有划分模块（没有home、search之分，只要仓库中有对应的getters就行）
  ...
  methods: {
    ...
    removeCategoryName() {
      // 把带给服务器的参数清空，还需要向服务器发请求
      this.searchParams.categoryName = undefined
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
      this.getData()
      // 地址栏也需要修改：进行路由跳转，如果路径中有params参数，路由跳转的时候应该带着
    ...
    // 排序操作
    changeOrder(flag) {
      // flag形参，是一个标记，代表用户点击的是综合（1）还是价格（2）【用户点击的时候传递进来】
  ...
  watch: {
    // 监听路由的信息是否发生变化（URL地址是否携带参数），如果发生变化，再次发起请求
    $route() {
      // 发请求前再次整理带给服务器的参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params)
      this.getData()
      // 每一次请求完毕，应该把相应的1、2、3级分类的id清空
      // 带给服务器的参数不是必选的：如果属性值为空字符串还是会把相应的字段带给服务器，但是把属性值改为undefined，当前的这个字段不会带给服务器
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    }
