      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins" @click="changeSkuNum('minus', -1, cart)">-</a>
            <input autocomplete="off" type="text" :value="cart.skuNum" minnum="1" class="itxt" @change="changeSkuNum('change', $event.target.value * 1, cart)" />
...
export default {
    ...
    async changeSkuNum(type, disNum, cart) {
      ...
      switch (type) {
        case 'add':
          ...
          break
        case 'minus':
          // 判断商品的个数大于1，才可以传递给服务器-1；如果商品个数小于等于1，传递给服务器0（原封不动）
          disNum = cart.skuNum > 1 ? -1 : 0
          break
        case 'change':
          // 如果用户输入的最终量非法（汉字或者负数），带给服务器的0（原封不动）
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0
          } else {
            // 合法，会出现小数，应该取整后减去原来的个数，得到的变化量传给服务器
  ...
  computed: {
    ...
    // 购买商品的总价
    totalPrice() {
      return this.cartInfoList.reduce(item => item.skuNum * item.skuPrice, 0)
    },
    // 全选复选框勾选状态
    isAllChecked() {
